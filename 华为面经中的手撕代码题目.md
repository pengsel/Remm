...
![img](https://img-blog.csdn.net/20160719110614857?watermark/2/text/aHR0cDovL2Jsb2cuY3Nkbi5uZXQv/font/5a6L5L2T/fontsize/400/fill/I0JBQkFCMA==/dissolve/70/gravity/Center)

最优公共子串的区别在于上面变为0,当xi!=yj时，c[i,j]=0。
...
```java
public static int useMoney(int money, int[] goods){
    int[] dp=new int[money+1];

    dp[0]=1;
    for (int i=0;i<goods.length;i++){
        for (int j=goods[i];j<=money;j++){
            if (j-goods[i]>=0) {
                dp[j]+= dp[j - goods[i]];
            }
        }
...
DP\[i][j]表示s1左边i个字符与s2左边j个字符的最大公共子序列，则有以下关系：
